...
//=============================================================================
// With some slight tweaks for Look Outside.
 ...
 * @default {"normal":"#22e488","mandatory":"#22e488","move":"#22e488"}
 * 
 * @param basicOk
 * @text 決定/ok
...
class InputDeviceBase extends I_ReadonlyMapper{
    ...
    createTemporaryMapper(){
        const tmp = new TemporaryMapper(this.currentMapper());
        return tmp;
    }
}
...
//            return this.currentGGG().getButtonByCode(code);
    ...
    defaultMapper_v2(){
        const tmp = new TemporaryMapper(this.defaultMapper());
        return tmp;
    }
    numButtons(){
...
const setting = (function(){
    ...
    const nintendo=createGamepadLayout("N","Switch Layout","B","A","Y","X");
    const playstation=createGamepadLayout("P","Playstation Layout","×","○","□","△");
    const xbox =createGamepadLayout("X","XBox Layout","A","B","X","Y");
    const numberGamepad =createButtonNumberLayout("Number","Numbered Buttons");
...
class InputConfigManager_T{
    ...
    createTemporalyGamepadMapper(){
        const mapper = new TemporaryMapper(this._saveData.gamepadConfig);
        return mapper;
    }
    applyGamepadConfig(){
...
class TemporaryMapperBase extends I_ReadonlyMapper{
    /**
     * 
     * @param {String} symbol 
     * @returns 
...
class TemporaryMapper extends TemporaryMapperBase{
    /**
     * @private
     * @param {InputMapperType} mapper 
     */
     ...
     */
    reset(mapper){
        this._map = TemporaryMapper.createMap(mapper);
    }
    /**
     * @param {Number} code 
     * @param {String} symbol 
...
class Window_InputConfigBase extends Window_Selectable_InputConfigVer{
     ...
     */
    hasSymbol(symbol){
        return this.temporaryMapper().hasSymbol(symbol);
    }
    canApplySetting(){
        return this.isValidMapper();
    }
    isValidMapper(){
        return this.temporaryMapper().isValidMapper();
    }
    resetMapper(){
        this.temporaryMapper().reset(this.defaultMapper());
    }
    resetMapper_V2(){
    ...
    cloneMapper(){
        return this.temporaryMapper().createNormalizedMapper();
    }
    updateHelp(){
...
class V8_Item_ApplyCommand extends V8Item_Command{
    ...
    constructor(command,mapper){
        super(command);
        this._mapper=mapper;
		SceneManager._scene.mappingIsCurrent=true
    }
    refresh(){
...
class Window_GamepadConfig_V8 extends Window_Selectable_InputConfigVer{
     ...
     */
    initialize(rect){
        this._tmpMapper= new TemporaryMapper(Input.gamepadMapper);
    ...
    makeV8Item(){
        ...
        baseList.push(this._resetCommand,this._layoutCommand,this._applyCommand,this._exitCommand);
...
class Scene_GamepadConfig_V8 extends Scene_MenuBaseMVMZ{
    ...
    onGamepadButton(){
        ...
        this._symbolWindow.selectSymbol(symbolString);
        this._symbolWindow.show();
...
class Scene_InputConfigBase_MA extends Scene_MenuBaseMVMZ{
    ...
    defaultMapper(){
        return this.mainWindow().defaultMapper();
    }
...
//        asw.refresh();
    ...
    selectSymbol() {
        const currentSymbol = this.mainWindow().currentSymbolString();
    ...
    mapperClass(){
        return this.mainWindow().temporaryMapper();
    }
...
function createButtonLayoutChangeCommand(){
    const mText = new MultiLanguageText("Layout Type","ボタン表記変更");
...
class Window_KeyConfig_MA extends Window_InputConfigBase {
    ...
    initializeMapper(){
        ...
        this._mapper217 = device.createTemporaryMapper();
         //new TemporaryMapper(device.currentMapper());
    }
    temporaryMapper(){
    ...
    symbolString(index){
        ...
        return this.temporaryMapper().findSymbolByCode(keyNumber);
    }
    /**
     * @param {Number} index 
     */
    ...
    symbolObjectFromKeyNumber(keyNumber){
        const symbol = this.temporaryMapper().findSymbolByCode(keyNumber)
        return symbolManager.findSymbol(symbol);
    }
...
class Scene_KeyConfig_MA extends Scene_InputConfigBase_MA{
    ...
    create() {
        super.create();
        this.createHelpWindow();
        this.createKeyboardConfigWindow();
        this.createSymbolListWindow();
    }
    onKeyLayoutOk(){
