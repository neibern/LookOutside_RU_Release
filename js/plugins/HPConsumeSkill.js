...
(function() {
  ...
  DataManager.processHpCost = function() {
    for (var i = 1; i < $dataSkills.length; i++) {
	  ...
	  var ratioRes = skill.meta.hp_ratio;
	  if(ratioRes){skill.hpRatio = ratioRes;}
	  else{skill.hpRatio = 0;}
    }
  };
...
Game_BattlerBase.prototype.canPaySkillCost = function(skill) {
  ...
  Game_BattlerBase.prototype.testForItemReq = function(skill){
		var allowFilter = true;
		if(skill.meta["DisableWithoutItem"])
		{
			var itemId = Number(skill.meta["DisableWithoutItem"]);
			if(itemId == 9999){itemId = gVr(938);}
			if (!Number.isNaN(itemId) && $dataItems[itemId])
			{
				if($gameParty.hasItem($dataItems[itemId], true)){return true;}
				else{return false;}
			}
			else
			{
  ...
  Game_BattlerBase.prototype.skillHpCost = function(skill) {
	var hpCostChk = skill.hpCost;
	hpCostChk +=  Math.floor(skill.hpRatio*this._hp);
    return hpCostChk;
  };
  ...
  Game_BattlerBase.prototype.paySkillCost = function(skill) {
	...
	var hpCostChk = this.skillHpCost(skill);
